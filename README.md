...
A WordPress VIP plugin that publishes posts and pages as Markdown and
provides `/llms.txt` discovery. Requires WordPress 6.8+, PHP 8.2+, and the VIP platform runtime.
...
- PHP 8.2 or newer
- The WordPress VIP platform runtime
...
Activate **Agent Ready Content** from the WordPress Plugins screen.

On WordPress VIP, follow the documentation for [activating plugins through code](https://docs.wpvip.com/how-tos/activate-plugins-through-code/).
...
Sites using plain permalinks advertise the query-string form instead.
Private content requires permission to read it. Password-protected content requires
...
without stored rewrite rules or a helper file.
WordPress’s `robots_txt` filter adds discovery where the platform permits it;
VIP test-domain crawler restrictions still apply. Actual edge-cache refresh
requires deployment verification.

## Build the settings assets
...
are public. A later registration for the same block name replaces the earlier one.
...
URL purge requests; this does not synchronously purge the edge cache. The second clears `/llms.txt`. Call before permanent deletion if a purge
needs the old permalink. A callback that only generates different output does
not itself invalidate a previously cached response.
...
empty lists. Objects and resources are unsupported. Mapping keys and strings are
quoted; Unicode and escaped line breaks survive a YAML round trip.
