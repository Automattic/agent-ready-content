...
The public behavior and supported extension contracts remain documented in the
[README](README.md).
...
| WordPress integration  | WordPress hooks, content conversion, access behavior, and PRC Bridge compatibility | `npm run test:integration`                        |
| Frontend static checks | TypeScript types, linting, formatting, and settings compilation                    | npm checks listed below                           |
| Manual UI              | The settings screen and public Markdown output in a real browser                   | Follow the Playwright instructions in `AGENTS.md` |
...
Start `wp-env` before running the integration suite. Its bootstrap loads both
the base plugin and PRC Bridge so their WordPress compatibility can be tested
together. Deployment checks cannot be fully reproduced by `wp-env`.
...
- To support a custom block, use the registry or block metadata described in
  [Extend the base](README.md#extend-the-base).
...
- When related data changes, identify the affected post IDs and use the public
  cache invalidators described in [Integration example](README.md#integration-example).
- Provider-specific compatibility belongs in a separately loaded plugin under
  [`integrations/`](integrations/README.md), not in the provider-neutral base.
  PRC Bridge is the current worked example.
...
the ZIP. The root package allowlist excludes development files and the entire
`integrations/` directory.

PRC Bridge has its own package metadata and ZIP command. From
`integrations/prc-bridge`, run:
...
This produces a standalone `prc-bridge.zip`. It is not included in or released
with the base plugin. Integrations keep their dependencies, versioning, release
process, and deployment separate from the base plugin.
